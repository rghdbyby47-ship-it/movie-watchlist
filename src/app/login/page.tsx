      ...
      <div className="retro-card" style={{ maxWidth: '420px', width: '100%' }}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ marginBottom: '28px' }}>
            ...
            <input className="retro-input" type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="••••••••" required />
          </div>
          ...
          <button type="submit" className="retro-btn retro-btn-cyan" style={{ width: '100%', fontSize: '11px' }} disabled={loading}>
            {loading ? 'LOGGING IN...' : 'LOGIN'}
          </button>
        </form>

        <div style={{ marginTop: '24px', textAlign: 'center', fontFamily: '"VT323", monospace', fontSize: '20px', color: 'var(--dim)' }}>
          NEW USER? <Link href="/signup" style={{ color: 'var(--yellow)' }}>REGISTER HERE</Link>
        </div>
      </div>
    </div>
  );
